...
static struct pib_ib_dev *pib_ib_add(struct device *dma_device, int ib_dev_id)
{
	...
	dev->ib_dev.uverbs_abi_ver	= PIB_IB_UVERBS_ABI_VERSION;
	...
	dev->ib_dev.query_device	= pib_ib_query_device;
	dev->ib_dev.query_port		= pib_ib_query_port;
	dev->ib_dev.get_link_layer	= pib_ib_get_link_layer;
	dev->ib_dev.query_gid		= pib_ib_query_gid;
	dev->ib_dev.query_pkey		= pib_ib_query_pkey;
	dev->ib_dev.modify_device	= pib_ib_modify_device;
	dev->ib_dev.modify_port		= pib_ib_modify_port;
	dev->ib_dev.alloc_ucontext	= pib_ib_alloc_ucontext;
	dev->ib_dev.dealloc_ucontext	= pib_ib_dealloc_ucontext;
	dev->ib_dev.mmap		= pib_ib_mmap;
	dev->ib_dev.alloc_pd		= pib_ib_alloc_pd;
	dev->ib_dev.dealloc_pd		= pib_ib_dealloc_pd;
	dev->ib_dev.create_ah		= pib_ib_create_ah;
	dev->ib_dev.destroy_ah		= pib_ib_destroy_ah;
	dev->ib_dev.create_srq		= pib_ib_create_srq;
	dev->ib_dev.modify_srq		= pib_ib_modify_srq;
	dev->ib_dev.query_srq		= pib_ib_query_srq;
	dev->ib_dev.destroy_srq		= pib_ib_destroy_srq;
	dev->ib_dev.post_srq_recv	= pib_ib_post_srq_recv;
	dev->ib_dev.create_qp		= pib_ib_create_qp;
	dev->ib_dev.modify_qp		= pib_ib_modify_qp;
	dev->ib_dev.query_qp		= pib_ib_query_qp;
	dev->ib_dev.destroy_qp		= pib_ib_destroy_qp;
	dev->ib_dev.post_send		= pib_ib_post_send;
	dev->ib_dev.post_recv		= pib_ib_post_recv;
	dev->ib_dev.create_cq		= pib_ib_create_cq;
	dev->ib_dev.modify_cq		= pib_ib_modify_cq;
	dev->ib_dev.resize_cq		= pib_ib_resize_cq;
	dev->ib_dev.destroy_cq		= pib_ib_destroy_cq;
	dev->ib_dev.poll_cq		= pib_ib_poll_cq;
	dev->ib_dev.req_notify_cq	= pib_ib_req_notify_cq;
	dev->ib_dev.get_dma_mr		= pib_ib_get_dma_mr;
	dev->ib_dev.reg_user_mr		= pib_ib_reg_user_mr;
	dev->ib_dev.dereg_mr		= pib_ib_dereg_mr;
	...
	dev->last_qp_num		= pib_random() & PIB_IB_QPN_MASK;
	...
	dev->behavior		= 0U;
#ifdef PIB_HACK_IMM_DATA_LKEY
	dev->imm_data_lkey	= PIB_IB_IMM_DATA_LKEY;
#endif
...
struct sockaddr *pib_get_sockaddr_from_lid(struct pib_ib_dev *dev, u8 port_num, struct pib_ib_qp *qp, u16 lid)
{
	int port_index;
...
static int __init pib_ib_init(void)
{
	...
	if (pib_create_switch(&pib_ib_easy_sw))
